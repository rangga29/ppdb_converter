...
class Sd extends CI_Controller
{
	...
	public function pdf_pendaftaran($slug_nama_lengkap)
	{
		...
		$dataPendaftaran = $this->SdModel->getSiswa($slug_nama_lengkap)->row();

		$bukti_pembayaran = 'http://localhost:8080/upload/bukti_pembayaran/sd/'.$dataPendaftaran->bukti_pembayaran.'';

		$pdf = new SDPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);

		$pdf->SetTitle('SD Pendaftaran '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		$pdf->SetSubject('SD Pendaftaran '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		...
		$pdf->Output('SD pendaftaran '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini.'.pdf');
	}
	...
	public function pdf_dapodik($slug_nama_lengkap)
	{
		...
		$tanggal_hari_ini = date('[Ymd]');
		$dataPendaftaran = $this->SdModel->getSiswa($slug_nama_lengkap)->row();
		$dataDapodik = $this->SdModel->getDapodik($dataPendaftaran->id)->row();
		...
		$pas_foto = 'http://localhost:8080/upload/pas_foto/sd/'.$dataDapodik->pas_foto.'';
		$scan_kk = 'http://localhost:8080/upload/scan_kk/sd/'.$dataDapodik->scan_kk.'';
		$scan_ak = 'http://localhost:8080/upload/scan_ak/sd/'.$dataDapodik->scan_ak.'';

		$pdf = new SDPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);

		$pdf->SetTitle('SD Dapodik '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		$pdf->SetSubject('SD Dapodik '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
	...
	public function surat_pernyataan($slug_nama_lengkap)
	{
		...
		$tanggal_hari_ini = date('[Ymd]');
		$dataPendaftaran = $this->SdModel->getSiswa($slug_nama_lengkap)->row();
		$dataPernyataan = $this->SdModel->getPernyataan($dataPendaftaran->id)->row();

		$tanda_tangan = 'http://localhost:8080/upload/tanda_tangan2/sd/'.$dataPernyataan->tanda_tangan.'';

		$pdf = new SDSURAT('P', PDF_UNIT, 'F4', true, 'UTF-8', false);

		$pdf->SetTitle('SD Surat Pernyataan '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		$pdf->SetSubject('SD Surat Pernyataan '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		...
		<table border="0" cellspacing="3" cellpadding="4">
			...
			<tr>
				...
				Dengan ini menyatakan bahwa jika anak kami diterima di SD KATOLIK
				...
				pendidikannya di <br>SD KATOLIK SANTA URSULA BANDUNG.
				</td>
			</tr>
			<br>
			<tr>
			...
			<tr>
				...
				sistem pembelajaran dan penilaian di SD KATOLIK SANTA URSULA
				BANDUNG;
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				calon peserta didik dikeluarkan dari SD KATOLIK SANTA URSULA
				BANDUNG.
				</td>
			</tr>
			<br>
			<tr>
				...
				kelancaran pendidikan putra/putri kami di lembaga pendidikan SD
	...
	public function surat_keuangan($slug_nama_lengkap)
	{
		...
		$tanggal_hari_ini = date('[Ymd]');
		$dataPendaftaran = $this->SdModel->getSiswa($slug_nama_lengkap)->row();
		$dataBeasiswa = $this->SdModel->getBeasiswa($dataPendaftaran->id)->row();
		$dataKeuangan = $this->SdModel->getKeuangan($dataPendaftaran->id)->row();
		...
		$tanda_tangan = 'http://localhost:8080/upload/tanda_tangan/sd/'.$dataKeuangan->tanda_tangan.'';

		$pdf = new SDSURAT('P', PDF_UNIT, 'F4', true, 'UTF-8', false);

		$pdf->SetTitle('SD Surat Keuangan '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		$pdf->SetSubject('SD Surat Keuangan '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini);
		...
		<table border="0" cellspacing="3" cellpadding="4">
			...
			<tr>
				<td colspan="3" style="text-align:justify">
				yang akan menempuh pendidikan di SD Santa Ursula Tahun
        ...
        $html .= '
			...
			<tr>
				<td colspan="3" style="text-align:justify">2. Segala bentuk pertanyaan dapat menghubungi kontak Panitia PPDB SD Santa
		...
		$pdf->Output('Sd keuangan '.$dataPendaftaran->no_registrasi.' '.$tanggal_hari_ini.'.pdf');
	}
}
...
class SDPDF extends TCPDF
{
	public function Header()
	{
		...
		$this->Cell(0, 13, 'SD KATOLIK SANTA URSULA', 0, true, 'C', 0, '', 0, false, 'M', 'M');
		$this->SetFont('times', 'B', 12);
		$this->Cell(0, 10, 'TERAKREDITASI A', 0, true, 'C', 0, '', 0, false, 'M', 'M');
		$this->SetFont('times', '', 12);
		$this->Cell(0, 10, 'Jalan Bengawan No. 2 Bandung 40114, Telepon (022) 7201774', 0, true, 'C', 0, '', 0, false, 'M', 'M');
		$this->SetFont('times', '', 12);
		$this->Cell(0, 13, 'Website: sd.santaursula-bdg.sch.id Email: sd.ursula.bdg@gmail.com', 0, true, 'C', 0, '', 0, false, 'M', 'M');
